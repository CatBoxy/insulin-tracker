...
| 1.2 | 2026-08-03 | Completed P0-6 (nightly Backblaze B2 backups with verified restore — all 24 tables pass), P0-10 (server-side measurement pagination, default 50/page, "Cargar más" on both patient and doctor views). Phase 0 complete except P0-8 (Sentry, deferred). |
| 1.3 | 2026-08-03 | Completed P1-4 (BP schema — systolic/diastolic columns with CHECK constraint, backfill, removed all regex parsing), P1-6 (abdominal circumference on doctor_indices), P1-7 (interaction tracking — last_interaction_at + app_sessions with 30min debounce, channel detection), P1-8 (server-side 20h measurement window enforcement, 429 response). All 3 migrations applied to prod. |
| 1.4 | 2026-08-03 | Completed P1-5 (lab results table with doctor-verified parsing flow). New `lab_results` table with UNIQUE constraint on (patient_id, timepoint, analyte, collected_on). Service layer with manual/parsed creation, single/bulk verify, unverified query. API routes for CRUD + verify. Doctor UI: "Verificar resultados" button on parsed attachments with analyte mapping, editable values, timepoint/date selection. Lab results panel with grouped table view, manual entry form, verified/unverified badges. Zod schemas for validation. |
| 1.5 | 2026-08-03 | Completed P1-2 (arm enforcement gate — checkArmGate() reads DB at call time, refuses control/withdrawn/non-enrolled, logs every refusal), P1-3 (feature flags — 6 seeded flags, per-arm settings, audit trail, isEnabled() resolver independent from arm gate), P1-9 (enrollment screening — admin UI with age ≥60 check, consent capture, participant code generation, arm change/withdrawal with reasons, full screening log). Phase 1 complete except P1-10 (admin panel consolidation). |
| 1.6 | 2026-08-03 | Completed P1-10 (admin panel — feature flags matrix with per-arm toggles/audit, study dashboard with enrollment stats and inactive participant alerts). Phase 1 complete. Started Phase 2: P2-1 (messaging schema — 5 tables: templates, schedule rules, messages, events, channels), P2-2 (channel abstraction — Channel interface, registry, unified sender with arm gate + channel check, idempotent event recording). All migrations applied to prod (40 tables total). |
...
| 2.1 | 2026-08-04 | Completed P0.5-3 (test infrastructure — vitest setup with path aliases, 3 test suites: arm gate tests for control/withdrawn/not-enrolled/intervention with refusal logging verification, scheduler tests for idempotency/inactivity-fire-once/no-template/arm-gate-refusal, cron-parser tests for wildcard/hour/minute/dow-list/dow-range/dom). CI updated to run tests before build — broken arm gate fails deploy. |
| 2.2 | 2026-08-04 | Completed P1.5-9 (reproducible arm allocation at enrollment). Migration adds allocation_method, allocation_sequence, allocated_at to study_participants. New getNextAllocation() uses alternating rule (odd=intervention, even=control). enroll() auto-assigns when no arm provided, records 'manual' method on override with audit reason. Zod schema makes arm optional. Admin UI arm selector defaults to "Automático (alternado)" with helper text; enrollment result shows assigned arm and method. |
